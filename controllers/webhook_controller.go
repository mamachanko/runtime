...
type WebhookConfiguration interface {
...
func AdmissionProjectorReconciler(c reconcilers.Config, name string, accessChecker rbac.AccessChecker) *reconcilers.AggregateReconciler[*WebhookConfiguration] {
	...
	return &reconcilers.AggregateReconciler[*admissionregistrationv1.MutatingWebhookConfiguration]{
		...
		Request: req,
		Reconciler: reconcilers.Sequence[*admissionregistrationv1.MutatingWebhookConfiguration]{
	...
	return &reconcilers.AdmissionWebhookAdapter[*unstructured.Unstructured]{
		...
		Reconciler: &reconcilers.SyncReconciler[*unstructured.Unstructured]{
			Sync: func(ctx context.Context, trigger *unstructured.Unstructured) error {
				...
				nsns, err := c.Tracker.GetObservers(trigger)
				if err != nil {
          log.Error(err, "unable to get observers")
          return err
				}
				for _, nsn := range nsns {
					rr := reconcile.Request{NamespacedName: nsn}
					log.V(2).Info("enqueue tracked request", "request", rr, "for", client.ObjectKeyFromObject(trigger), "dryRun", req.DryRun)
...
func LoadServiceBindings(req reconcile.Request) reconcilers.SubReconciler[WebhookConfiguration] {
	return &reconcilers.SyncReconciler[*WebhookConfiguration]{
		Name: "LoadServiceBindings",
		Sync: func(ctx context.Context, _ WebhookConfiguration) error {
			c := reconcilers.RetrieveConfigOrDie(ctx)
...
func InterceptGVKs() reconcilers.SubReconciler[*admissionregistrationv1.MutatingWebhookConfiguration] {
	return &reconcilers.SyncReconciler[*admissionregistrationv1.MutatingWebhookConfiguration]{
		Name: "InterceptGVKs",
		Sync: func(ctx context.Context, _ *admissionregistrationv1.MutatingWebhookConfiguration) error {
...
func WebhookRules(operations []admissionregistrationv1.OperationType, accessChecker rbac.AccessChecker) reconcilers.SubReconciler[*admissionregistrationv1.MutatingWebhookConfiguration] {
	return &reconcilers.SyncReconciler[*admissionregistrationv1.MutatingWebhookConfiguration]{
		Name: "WebhookRules",
		Sync: func(ctx context.Context, _ *admissionregistrationv1.MutatingWebhookConfiguration) error {
			log := logr.FromContextOrDiscard(ctx)
